...
**Review actions** — Approve, Request Changes, Comment. The Comment action posts a regular PR comment via `gh pr comment`. Request Changes allows an optional comment; if you provide nothing, a default message is posted because both APIs require a review body.

**Merge action** — with a strategy picker. GitHub offers *Merge commit*, *Squash*, and *Rebase*; Bitbucket offers *Merge commit*, *Squash*, and *Fast-forward*. Branch protection rules and required status checks still apply — if the PR cannot merge, the platform refuses and branchdiff surfaces the error inline.
