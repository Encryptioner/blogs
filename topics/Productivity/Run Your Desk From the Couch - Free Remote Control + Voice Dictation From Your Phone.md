...
If it's still refused after that: check `ufw status verbose` actually shows the port-5900 rule as active, confirm phone and desktop are on the *same* WiFi — not just by the icon but on the phone's WiFi details page (SSID matches, and the *gateway* is your router's LAN address; a silent fallback to mobile data or a guest band puts the phone on a different path) — and re-check the desktop's LAN IP hasn't drifted from a DHCP re-lease (`hostname -I` is the one-glance version: ignore the `172.x.x.x` Docker/VM bridges, read the `192.168.x.x` one). And learn the distinction that saves the most time: **"timed out" means the packets never arrived** (phone on the wrong network, guest/AP isolation on the router, or a VPN running on the phone) while **"refused" means they arrived and nothing was listening** — server-side, above. The ten-second instrument that settles it: `journalctl --user -f | grep -i x11vnc` while you tap Connect on the phone. Nothing logged = traffic isn't arriving; `Got connection from client <ip>` then an auth failure = the network is fine, it's the password.
